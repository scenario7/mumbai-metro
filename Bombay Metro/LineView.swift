                    ...
                    ZStack(alignment:.topLeading) {
                        ...
                        VStack(spacing:40){
                            ...
                            HStack(spacing:40) {
                                    ...
                                    VStack(alignment:.trailing, spacing: 21){
                                        ForEach(line.stations) { station in
                                            VStack(alignment:.trailing, spacing:20) {
                                                ...
                                                if (station.id != line.stations.last?.id){
                                                    if let time = station.secToNext{
                                                        ZStack(alignment:.center){
                                                            LinearGradient(colors: [line.color,.blue], startPoint: .topLeading, endPoint: .bottomTrailing)
                                                                .mask {
                                ...
                                VStack(spacing:-10){
                                    ForEach(line.stations) { station in
                                        if(station.id == line.stations.first?.id){
                                            VStack(spacing:-10) {
                                                ZStack {
                                                    Circle()
                                        ...
                                        else if(station.id == line.stations.last?.id) {
                                            VStack(spacing:-10) {
                                                Rectangle()
...
struct LineView_Previews: PreviewProvider {
  ...
  func timeString(time: TimeInterval) -> String {
      ...
      return String(format: "%2i:%02i min", minute, second)
  }
